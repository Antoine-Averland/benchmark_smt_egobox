...
def create_chart(matrix_dimensions, list_num_points, data):
    for npoints in list_num_points:
        ...
        for i, program in enumerate(['SMT', 'EGObox']):
            matrix_means = [np.mean(data[program][matrix][npoints]) for matrix in matrix_dimensions]
            matrix_stddevs = [np.std(data[program][matrix][npoints]) for matrix in matrix_dimensions]
